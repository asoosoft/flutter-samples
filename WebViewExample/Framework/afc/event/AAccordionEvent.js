...
class AAccordionEvent extends AEvent
{
	constructor(acomp)
	{
		super(acomp);
...
window.AAccordionEvent = AAccordionEvent;
...
AAccordionEvent.prototype._select = function(selItem)
{
	...
	$menu.bind(acc.option.showEvent, async function(e)
	{
		//mouseover delay 값이 지정된 경우 delay 후에 이벤트가 발생하도록 처리
		if(acc.option.mouseOverEventDelay>0)
		{
			if(!isTrigger) //트리거로 이벤트를 발생시켰는지
			{
				...
				acc.option.mouseOverEventDelay);
		...
		if(selItem.url && !selItem.contents.view)	//이미 로드되어져 있는 경우는 제외
		{
			...
			var aview = await AView.createView(selItem.contents, selItem.url, acc);
				
			acc._afterCreated(aview, selItem.data);
		}
