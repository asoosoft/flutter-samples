...
//--------------------------------------------------------------------
class AFlexView extends AComponent
{
	constructor()
	{
		super()
...
AFlexView.prototype.insertView = async function(view, index)
{
	var $item = $('<div></div>');
	...
	//else $(this.views[index]._item).before($item);
	...
	{
		$(this.views[index]._item).before($item);
...
AFlexView.prototype._callSubActiveEvent = function(funcName, isFirst)
{
	for(var i=0; i<this.views.length; i++)
	{
