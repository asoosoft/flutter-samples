...
class ASelectBox extends AComponent
{
	constructor()
	{
		super()
...
window.ASelectBox = ASelectBox

ASelectBox.CONTEXT = 
{
    tag: '<select data-base="ASelectBox" data-class="ASelectBox" class="ASelectBox-Style"><option value="value1">item1</option></select>',
...
ASelectBox.prototype.setData = function(dataArr)
{
	this.removeAll();
	
	for(var i=0; i<dataArr.length; i++)
		this.addItem(dataArr[i][0], dataArr[i][1], dataArr[i][2]);
};

ASelectBox.prototype.getData = function()
{
	var item = this.getSelectedItem();
	if(item)
	{
		if(item.data == undefined) return item.value;
...
ASelectBox.prototype.setQueryData = function(dataArr, keyArr, queryData)
{
	...
	if(!this.applyType || this.applyType=='add')
	{
		...
		for(var i=0; i<dataArr.length; i++)
			this.addItem(dataArr[i][keyArr[1]], dataArr[i][keyArr[0]], dataArr[i][keyArr[0]]);
	
	}
	else if(this.applyType=='select')
	{
		//매핑한 value 값을 얻어 선택되도록 한다.
		this.selectItemByValue(dataArr[0][keyArr[0]]);
	
	}
	else if(this.applyType=='remove')
	{
		this.removeItemByValue(dataArr[0][keyArr[0]]);
	}
