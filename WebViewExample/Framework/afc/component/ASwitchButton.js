...
class ASwitchButton extends AComponent
{
	constructor()
	{
		super()
...
window.ASwitchButton = ASwitchButton
...
ASwitchButton.prototype.setSwitchOnStyle = function(backOnStyle)
{
	if(backOnStyle) 
	{
		...
		if(this.isDev()) this.setValue(true);
	}
};
...
ASwitchButton.prototype.setSwitchOffStyle = function(backOffStyle)
{
	if(backOffStyle)
	{
		...
		if(this.isDev()) this.setValue(false);
	}
};
...
ASwitchButton.prototype.setData = function(data)
{
	this.setValue(data);
};

ASwitchButton.prototype.getData = function()
{
...
ASwitchButton.prototype._setDataStyleObj = function(styleObj)
{
	for(var p in styleObj)
	{
		...
		{
			if(p == "data-style-on") this.setSwitchOnStyle(styleObj[p]);
			else if(p == "data-style-off") this.setSwitchOffStyle(styleObj[p]);
