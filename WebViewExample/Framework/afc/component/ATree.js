...
ATree.prototype.init = function(context, evtListener)
{
    ...
    this.setOption(
    {
        ...
        isFullSelect: true,        //아이템 선택시 선택표시가 라인 전체로 표시된다.
...
ATree.prototype.startUseHistory = function()
{
	...
	this.undoStack = AUtil.makeStack(this.$ele);
	this.redoStack = AUtil.makeStack(this.$ele);
};
...
ATree.prototype._getLabelItem = function(upperItem)
{
    ...
    else return $(upperItem).children('label')
};

ATree.prototype.getSelectedParent = function(mItem)
{
    return this._getLabelItem(mItem.pItem).hasClass(this.selectStyle);
};
...
ATree.prototype.setItem = function(item, itemInfo)//itemInfo -> name, data, icon, action
{
	for(var p in itemInfo)
	{
		...
		else if(p=='name') $(item).children('label').eq(0).text(itemInfo.name);
...
ATree.prototype.moveItem = function(dropItem, mItems, isInsertAfter)
{
	...
	for(var i=0,len=mItems.length; i<len; i++)
	{
	    ...
	    if(this._getLabelItem(mItems[i].pItem).hasClass(this.selectStyle)) continue;
...
{
	var commentEle = $(item).children('label')[1];
	if(comment)
	{
		if(commentEle)
		{
		...
		else $(item).children('label').eq(0).after('<label title="'+comment+'" style="padding:3px;">['+comment+']</label>');
	}
...
{
    let item = $(ATree.liFormat);

	if(this.option.isFullSelect)
	{
		item.append('<div class="tree_-fullrow"></div>');
	}
	...
	*/
    ...
    {
		var $icon = $('<span></span>');
		item[0].iconEle = $icon[0];
		this.changeIcon(item[0], itemInfo.icon, itemInfo.iconType);
        item.append($icon);
    }
    ...
    if(this.option.isDraggable)
    {
    	...
    	if(itemInfo.pItem)
    	{
			this._itemDragManage(itemDom);
		}
		
		if(itemInfo.isDrop)
		{
			this._itemDropManage(this._getLabelItem(itemDom)[0]);
		}
    }
...
ATree.prototype.rename = function(item, name)
{
	...
	item.name = name;
	$(item).children('label').eq(0).text(name);
};
...
ATree.prototype._insertHelper = function(pItem, isExpand)
{
	...
	if (subTree.length == 0) 
	{
		...
		this._expandEnable(parentItem, subTree, isExpand);
	}
...
ATree.prototype._deleteHelper = function(pItem)
{
	...
	{
		subTree.remove();
		this._expandDisable(parentItem);
	}
};
...
ATree.prototype.clearSelected = function()
{
    ...
    {
    	this._getLabelItem(this.selectedItems[j]).removeClass(this.selectStyle);
    }
...
ATree.prototype.deselectItem = function(item)
{
	...
	this._getLabelItem(item).removeClass(this.selectStyle);
	/*
...
ATree.prototype.selectItem = function(item, isMulti, e)
{
	...
	this.$ele.find('.'+this.selectStyle).each(function(i, el){
		...
		let node = thisObj.option.isFullSelect ? el : el.parentNode

		if(thisObj.getSelectedIndex(node) < 0)
			thisObj.selectedItems.push(node);
	});
...
ATree.prototype._itemSelectRange = function(oneItem)
{
	let totLab = this.option.isFullSelect ? this.$root.find('li') : this.$root.find('label');
	let oneLab = this._getLabelItem(oneItem)[0];
	let twoLab = this._getLabelItem(this.lastSelectedItem)[0];
	...
	if(nextItem)
	{
		...
		this._itemSelectRange(nextItem, lastItem);
	}
	*/
};
...
ATree.prototype.onElementDrop = function(dndManager, event, dragElement)
{
    ...
    var $dropItem = $(dropEle), parent = $dropItem.parent()[0];

    //드롭하는 대상이 선택되어 있거나, 부모가 선택되어 있으면 이동시키지 않음
    if($dropItem.hasClass(this.selectStyle) || this._getLabelItem(parent.pItem).hasClass(this.selectStyle)) return;
...
{
	var $icon = $(item.iconEle);
	$icon.addClass(this.iconMap);
	$icon.css(
	{
...
{
	return Boolean($(item).children('.expanded')[0]);
}